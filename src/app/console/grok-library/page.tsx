      ...
      {loading ? (
        ...
        <div className="text-center py-12">
          ...
          <p className="text-zinc-600 text-xs mt-1">
            Generate designs in the Product Creator or upload images above
          </p>
        </div>
      ) : (
            ...
            >
              ...
              {editingId !== item.id && movingId !== item.id && (
                <div className="absolute top-1.5 right-1.5 flex gap-1 opacity-0 group-hover:opacity-100 transition">
                  <button
                    onClick={() => {
                      const params = new URLSearchParams({ ref: item.url });
                      if (item.prompt) params.set("prompt", item.prompt);
                      if (item.product_type) params.set("product", item.product_type);
                      window.location.href = `/console/design-studio?${params.toString()}`;
                    }}
                    className="rounded-full bg-purple-600/90 p-1.5 text-white hover:bg-purple-500 transition"
                    title="Use in Product Creator"
                  >
