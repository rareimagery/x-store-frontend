...
export default function MyPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-6 space-y-6">
      ...
      {/* ============================================================ */}
      {/* Full profile section (when all done)                         */}
      {/* ============================================================ */}
      {allDone && profile && (
        <>
          {/* Metrics */}
          {profile.metrics && (
            <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-5">
              <h3 className="text-sm font-semibold text-white mb-3">X Metrics</h3>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                {[
                  { label: "Engagement", value: `${(profile.metrics.engagement_score * 100).toFixed(1)}%` },
                  { label: "Avg Likes", value: fmt(profile.metrics.avg_likes) },
                  { label: "Avg Retweets", value: fmt(profile.metrics.avg_retweets) },
                  { label: "Avg Views", value: fmt(profile.metrics.avg_views) },
                ].map((m) => (
                  <div key={m.label} className="rounded-lg bg-zinc-800/60 border border-zinc-700/50 p-2.5 text-center">
              ...
              {profile.metrics.top_themes.length > 0 && (
                <div className="flex flex-wrap gap-1 mt-3">
                  {profile.metrics.top_themes.slice(0, 6).map((t) => (
                    <span key={t} className="rounded-full bg-purple-900/30 border border-purple-700/40 px-2 py-0.5 text-[9px] text-purple-300">{t}</span>
                  ))}
                </div>
              )}
            </div>
          ...
          {profile.topPosts.length > 0 && (
            <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-5">
              <h3 className="text-sm font-semibold text-white mb-3">Recent Posts ({profile.topPosts.length})</h3>
              <div className="space-y-2">
                {profile.topPosts.slice(0, 5).map((post) => (
                  <div key={post.id} className="rounded-lg bg-zinc-800/60 border border-zinc-700/50 p-3">
                    <p className="text-xs text-zinc-300 line-clamp-2">{post.text}</p>
                    <div className="flex gap-3 mt-1.5 text-[9px] text-zinc-600">
                      <span>{fmt(post.likes)} likes</span>
                      <span>{fmt(post.retweets)} RTs</span>
          ...
          {/* Top followers */}
          {profile.topFollowers.length > 0 && (
            <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-5">
              <h3 className="text-sm font-semibold text-white mb-3">Top Followers</h3>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                {profile.topFollowers.slice(0, 8).map((f) => (
                  <a key={f.username} href={`https://x.com/${f.username}`} target="_blank" rel="noopener noreferrer"
                    className="rounded-lg bg-zinc-800/60 border border-zinc-700/50 p-2.5 hover:border-purple-500/50 transition flex items-center gap-2">
                    <div className="h-7 w-7 rounded-full bg-zinc-700 overflow-hidden shrink-0">
                      {f.profile_image_url ? (
                        <img src={f.profile_image_url} alt="" className="w-full h-full object-cover" />
                      ) : (
                        <div className="w-full h-full flex items-center justify-center text-[9px] text-zinc-500">{f.username[0]?.toUpperCase()}</div>
                      )}
                    </div>
                    <div className="min-w-0">
                      <p className="text-[10px] font-medium text-zinc-200 truncate">{f.display_name}</p>
                      <p className="text-[9px] text-zinc-600">{fmt(f.follower_count)}</p>
                    </div>
                  </a>
                ))}
              </div>
