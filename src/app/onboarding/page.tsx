...
export default function OnboardingWizard() {
  ...
  const handleClaimAndCreate = async () => {
    ...
    setError(null);
    try {
      await createCreatorSite(effectiveProfile, DEFAULT_TEMPLATE_ID, subdomain);
      router.push('/console/my-page');
    } catch (err: any) {
  ...
  const canContinue = effectiveProfile.handle && subdomain.length >= 3 && slugStatus === 'available' && !creating;
    ...
    <div className="flex min-h-screen items-center justify-center bg-zinc-950 p-8 text-white">
      <div className="w-full max-w-3xl">
        ...
        <div className="space-y-8">
          <h2 className="text-3xl font-semibold">Claim Your Store</h2>
          ...
          >
            {creating ? 'Creating your store...' : 'Claim & Create Store'}
          </button>
        </div>
      </div>
    </div>
